        ...
        for j in range(0, len(max_plu)):
            ...
            for x in range(0, len(index)):
                ...
                if len(index) < 2:
                    ...
                    index_min = [index for index, v in enumerate(flatten_ballot) if A[index[x] - 1] in v]
                    print(index_min)
...
if __name__ == "__main__":
    for iteration in range(50):
        ...
        for idx in range(0, n):  # change second parameter for number of voters
            vote = np.random.permutation(A_example)
            ballot_example = np.append(ballot_example, [vote], axis=0)
